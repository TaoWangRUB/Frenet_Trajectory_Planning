...
void Plot_state(const VectorXd& mu, const MatrixXd& sigma, const Mapper& mapper, const vector<bool>&observedLandmarks, const vector<RadarReading>& Z) {
    ...
    for(int i = 0; i < observedLandmarks.size(); i++) {
        if(observedLandmarks[i]) {
            ...
            if(!islabeled) {
                auto settings = o_settings;
                settings.emplace("label", "observation");
    ...
    for(const auto i : Z) {
        ...
        X.push_back(mu(2*i.id+1));
        Y.push_back(mu(2*i.id+2));
        plt::plot(X, Y, "k");
        plt::legend();
    }
...
void Clear(int t = 0) {
    plt::clf();
    // preseting of the plot
    plt::xlim(0, 70);
    plt::ylim(-15,20);
    stringstream ss;
    ss << setfill('0') << setw(3) << t/10.;
    plt::title("Frenet trajectory planning " + ss.str() + "s");
    plt::xlabel("X (m)");
    plt::ylabel("Y (m)");
    plt::grid(true);
}
...
void Save(string path = "./test.png") {
        plt::save(path);
}
